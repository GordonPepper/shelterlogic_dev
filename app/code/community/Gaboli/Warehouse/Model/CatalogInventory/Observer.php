<?php
/**
 * Class Gaboli_Warehouse_Model_CatalogInventory_Observer
 */
class Gaboli_Warehouse_Model_CatalogInventory_Observer extends Mage_CatalogInventory_Model_Observer
{
    /**
    ...
    public function checkQuoteItemQty($observer)
    {
        ...
        if($options && $qty > 0) {
            ...
            foreach ($options as $option) {
                ...
                if(!$stockItem instanceof Mage_CatalogInventory_Model_Stock_Item && !$stockItem instanceof Gaboli_Warehouse_Model_CatalogInventory_Stock_Item) {
                    Mage::throwException(
    ...
    public function refundOrderInventory($observer)
    {
        ...
        foreach ($creditmemo->getAllItems() as $item) {
            ...
            if($return) {
                ...
                $locationIds = Mage::helper('gaboli_warehouse/location')->getPrioritizedLocationsForOrderQuoteItem($orderId, $quoteItemId);


                $sourceCollection = Mage::getModel('gaboli_warehouse/order_stock_source')
                    ->getCollection()
    ...
    public function _loopCancelItem($args)
    {
        ...
        $stockItem = Mage::getModel('gaboli_warehouse/stock')->loadByProduct($locationId, $productId);

        if($this->inventoryToRestock < $qtyFromLocation) {
            $orderStockSource = Mage::getModel('gaboli_warehouse/order_stock_source')->load($row['id']);
