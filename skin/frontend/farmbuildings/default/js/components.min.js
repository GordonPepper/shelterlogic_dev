...
angular.module("mm.foundation.accordion", []).constant("accordionConfig", {
        ...
        link: function(scope, element, attrs, controller) {
            var parent = element.parent(), dropdown = angular.element($document[0].querySelector(scope.dropdownToggle)), parentHasDropdown = function() {
                ...
                !elementWasOpen && !element.hasClass("disabled") && !element.prop("disabled")) {
                    ...
                    dropdown.css(css), element.addClass("expanded"), parentHasDropdown() && parent.addClass("hover"), 
                    openElement = element, closeMenu = function() {
                        $document.off("click", closeMenu), dropdown.css("display", "none"), element.removeClass("expanded"), 
                        closeMenu = angular.noop, openElement = null, parent.hasClass("hover") && parent.removeClass("hover");
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            scope.windowClass = attrs.windowClass || "", $timeout(function() {
                scope.animate = !0, element[0].querySelectorAll("[autofocus]").length > 0 ? element[0].querySelectorAll("[autofocus]")[0].focus() : element[0].focus();
            });
        }
    };
    ...
    return $rootScope.$watch(backdropIndex, function(newBackdropIndex) {
        ...
        faux.remove();
        var scrollY = $window.pageYOffset || 0, openAt = scrollY + marginTop, angularDomEl = angular.element('<div modal-window style="visibility: visible; top:' + openAt + 'px;"></div>').attr({
    ...
    return {
        ...
        link: function(scope, element) {
            var topbar = scope.topbar = element, topbarContainer = topbar.parent(), body = angular.element($document[0].querySelector("body")), lastBreakpoint = mediaQueries.topbarBreakpoint(), isSticky = scope.isSticky = function() {
                ...
                if (scope.stickyTopbar && scope.isSticky()) {
                    ...
                    $window.pageYOffset > distance && !topbarContainer.hasClass("fixed") ? (topbarContainer.addClass("fixed"), 
                    body.css("padding-top", scope.originalHeight + "px")) : $window.pageYOffset <= distance && topbarContainer.hasClass("fixed") && (topbarContainer.removeClass("fixed"), 
            ...
            }), angular.element($window).bind("resize", onResize), angular.element($window).bind("scroll", onScroll), 
            scope.$on("$destroy", function() {
                angular.element($window).unbind("scroll", onResize), angular.element($window).unbind("resize", onScroll);
    ...
    return {
        ...
        transclude: !0,
        link: function(scope, element, attrs, ctrls) {
            var $titleLi, topBar = ctrls[0], hasDropdown = ctrls[1], $link = angular.element(hasDropdown.triggerLink), url = $link.attr("href");
    ...
    function setCurrentStep(step) {
        currentIndex = step, storeCurrentStep();
    }
    var currentIndex = getStoredCurrentStep(), steps = {};
